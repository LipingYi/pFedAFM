...
from experiments.FLHetero.Models.CNNs import CNN_1_hetero_AFM, CNN_2_hetero_AFM,CNN_3_hetero_AFM,CNN_4_hetero_AFM,CNN_5_hetero_AFM,CNN_5_homo_AFM
...
def test_acc_local(net_large, net_small, net_alpha, testloader,criteria):
    # net.eval()
    with torch.no_grad():
        test_acc = 0
        num_batch = 0
        ...
        for batch in testloader:
            ...
            large_pred, mix_rep = net_large(img, torch.zeros(500).to(device), torch.ones(500).to(device))
...
def train(data_name: str, data_path: str, classes_per_node: int, num_nodes: int, fraction: float,
    ...
    orininal_file = str(save_path / f"Hetero_FedAFM_mixAcc_lr{lr_alpha}_N{num_nodes}_C{fraction}_R{steps}_E{epochs}_B{bs}_NonIID_{data_name}.csv")
    ...
    with open(orininal_file, 'w', newline='') as file:
        ...
        for step in step_iter:  # step is round
            ...
            for c in select_nodes:
                ...
                # # simultaneously train local and global NN
                ...
                trained_loss1, trained_acc1 = test_acc(net_large, net_small, alpha_model, nodes.test_loaders[node_id],criteria)
            ...
            results = [mean_small_global_loss, mean_small_global_acc, mean_large_trained_loss, mean_large_trained_acc, mean_clear_local_large_loss, mean_large_clear_local_large_acc] + [round(i,4) for i in PM_large_acc.values()] + [i.data.item() for i in Alpha_mean.values()]  #+ [i for i in LR_gate.values()]
    ...
    new_file = str(save_path / f"Done_Hetero_FedAFM_mixAcc_lr{lr_alpha}_N{num_nodes}_C{fraction}_R{steps}_E{epochs}_B{bs}_NonIID_{data_name}.csv")
